# Changelog

## Unreleased

- Added public research notes covering OTA discovery, firmware container
  structure, activation findings, and the patch ladder behind the released
  firmware.
- Added protocol notes for normal BLE commands, realtime sensor packets, raw
  motion decoding, and QRing DFU frames.
- Added sanitised packet examples for battery, raw motion, realtime sensors,
  quiet sensors, DFU start, and stock OTA metadata.
- Added GitHub issue templates for flashing failures, MIDI issues, and
  compatibility reports.
